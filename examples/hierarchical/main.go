...
func main() {
	// Hierarchical: app > mode (compound) > locked/unlocked
	mb := primitives.NewMachineBuilder("hierarchical", "app")
	app := mb.Compound("app").WithInitial("mode")
	mode := app.Compound("mode").WithInitial("locked")
	mode.Atomic("locked").Transition("PIN", "unlocked")
	mode.Atomic("unlocked").Transition("TIMEOUT", "locked")
	...
	for cycles < 6 {
		select {
		case <-ticker.C:
			ev := primitives.NewEvent("PIN", nil)
			if cycles%2 == 1 {
				ev = primitives.NewEvent("TIMEOUT", nil)
			}
